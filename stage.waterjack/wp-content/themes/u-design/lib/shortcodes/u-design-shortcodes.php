...
function read_more_func( $atts ) {
    ...
    $align_class = ( $atts['align'] == 'right' ) ? '-align-right': '-align-left';
...
function button_func( $atts ) {
    ...
    $target = ( $atts['target'] == '_blank' ) ? ' target="_blank"' : '';
    $style_class = ( $atts['style'] == 'dark' ) ? ' dark-button': ' light-button';
    $align_class = '';
...
function small_button_func( $atts ) {
    ...
    $target = ( $atts['target'] == '_blank' ) ? ' target="_blank"' : '';
    $style_class = ( $atts['style'] == 'dark' ) ? ' small-dark-button': ' small-light-button';
    $align_class = '';
    $before = $after = '<div class="clear"></div>';
    if( $atts['align'] == 'right' ) {
        $align_class = ' align-btn-right';
 ...
 * 
 * Usage: [round_button text="Read more..." style="light" title="Nice Button" url="http://www.example.com/" align="left" target="_blank"]
 * 
 */
function round_button_func( $atts ) {
    ...
    $target = ( $atts['target'] == '_blank' ) ? ' target="_blank"' : '';
    $style_class = ( $atts['style'] == 'dark' ) ? ' dark-round-button': ' light-round-button';
    $align_class = '';
    $before = $after = '<div class="clear"></div>';
    if( $atts['align'] == 'right' ) {
        $align_class = ' align-btn-right';
 ...
 * 
 * Usage (custom): [message type="custom" width="100%" start_color="#FFFFFF" end_color="#EEEEEE" border="#BBBBBB" color="#333333" align="left"]Your info message goes here...[/message]
...
function message_box_func( $atts, $content = null ) {
	...
	$html = '<div class="'.$atts['type'].'" style="background:-moz-linear-gradient(center top , '.$atts['start_color'].', '.$atts['end_color'].') repeat scroll 0 0 transparent;
	...
	$html = '<div class="'.$atts['type'].'" style="background-color:'.$atts['bg_color'].'; color:'.$atts['color'].';"><div class="inner-padding">'.do_shortcode( $content ).'</div></div>';
    } else {
	$html = '<div class="'.$atts['type'].'"><div class="msg-box-icon">'.do_shortcode( $content ).'</div></div>';
    }
    return $html;
}
...
function pullquote_func( $atts, $content = null ) {
    ...
    $align = ( $atts['style'] == 'right' ) ? 'alignright' : 'alignleft';
    $quote_color = ( $atts['quote'] === 'light' ) ? ' bq-light' : '';
...
function pullquote2_func( $atts, $content = null ) {
    ...
    $align = ( $atts['style'] == 'right' ) ? 'alignright' : 'alignleft';
    $quote_color = ( $atts['quote'] === 'light' ) ? ' bq-light-2' : ' bq-dark-2';
    return '<blockquote class="'.$align.$quote_color.'">'.do_shortcode( $content ).'</blockquote>';
}
...
function tab_func( $atts, $content = null ) {
    ...
    $single_tab_array[] = array( 'title' => $atts['title'], 'content' => trim( do_shortcode( $content ) ) );
...
function accordion_toggle_func( $atts, $content = null ) {
    $atts = shortcode_atts( array(
	    'title'      => '',
    ), $atts, 'accordion_toggle' );
    global $single_accordion_toggle_array;
    $single_accordion_toggle_array[] = array( 'title' => $atts['title'], 'content' => trim( do_shortcode( $content ) ) );
...
function custom_frame_left_func( $atts, $content = null ) {
    $atts = shortcode_atts( array(
	    'shadow' => 'off',
    ), $atts, 'custom_frame_left' );
    $shadow_class = ( $atts['shadow'] == 'off' ) ? '': ' frame-shadow';
 ...
 * 
 */
function udesign_recent_posts_func( $atts ) {
...
function content_block_func( $atts, $content = null ) {
    ...
    $bg_fixed = ( $atts['bg_fixed'] == 'yes' ) ? 'fixed': 'scroll';
    ...
    ob_start(); ?>
            <style type="text/css">
                #content-block-background-<?php echo $unique_id; ?> { background-image: url(<?php echo $atts['bg_image']; ?>); background-position: <?php echo $atts['bg_position']; ?>; background-repeat: <?php echo $atts['bg_repeat']; ?>; background-color: <?php echo $atts['bg_color']; ?>; background-attachment: <?php echo $bg_fixed; ?>; background-size: <?php echo $atts['bg_size']; ?>; }
                #content-block-body-<?php echo $unique_id; ?> { padding: <?php echo $atts['content_padding']; ?>; color: <?php echo $atts['font_color']; ?>; }
                .content-block-body { margin-left: auto; margin-right: auto; position: relative; }
            </style>
<?php       if( $atts['max_bg_width'] == "yes" ) : ?>
                <style type="text/css">
...
<?php       if( $atts['parallax_scroll'] == "yes" ) : ?>
                <script type="text/javascript">
                    // <![CDATA[
                    jQuery(document).ready(function($){
...
<?php       endif; ?>
            ...
            <div class="clear"></div>
            <div id="content-block-background-<?php echo $unique_id; ?>" class="content-block-background <?php echo $atts['class']; ?>">
