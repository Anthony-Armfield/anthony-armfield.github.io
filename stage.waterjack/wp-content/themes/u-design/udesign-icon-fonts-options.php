...
function udesign_icon_fonts_options_menu() {
	global $submenu;
	$udesign_icon_fonts_admin_page =  add_submenu_page('udesign_options_page', __( 'Icon Fonts', 'udesign' ), __( 'Icon Fonts', 'udesign' ), who_can_edit_udesign_theme_options(), 'udesign_icon_fonts_options_page', 'icon_fonts_options_page_callback');
...
function udesign_icon_font_func( $atts ) {
    $atts = shortcode_atts( array(
	    ...
	    'color' => '',
	    'size' => '',
    ), $atts, 'udesign_icon_font' );
    $color = ( $atts['color'] ) ? "color:{$atts['color']};" : '';
    $size = ( $atts['size'] ) ? "font-size:{$atts['size']};" : '';
    $inline_styles = ( $color || $size ) ?  ' style="'.$color . $size.'"' : '';
    $html = '<i class="'.$atts['name'].'"'.$inline_styles.'></i>';
    return $html;
}
