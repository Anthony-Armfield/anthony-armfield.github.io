...
define( 'UDESIGN_VERSION', '2.13.10' );
...
// Set the WordPress Content Width. Content Width is a feature in themes that allows you to set the maximum allowed width for videos, images, and other oEmbed content in a theme.
function udesign_set_content_width() {
    global $content_width, $udesign_options;
    if ( !isset( $content_width ) ) {
        if ( isset( $udesign_options['udesign_content_width'] ) ) {
            $content_width = $udesign_options['udesign_content_width'];
        } else {
            $content_width = 600;
        }
    }
}
add_action( 'after_setup_theme', 'udesign_set_content_width' );
...
add_filter('excerpt_more', 'remove_default_excerpt_more');
...
/**
 * Include the "Get the Image" functionality
 */
include ( trailingslashit( get_template_directory() ) . 'lib/u-design-get-the-image/get-the-image.php' );
...
if ( !is_admin() && $udesign_options['exclude_portfolio_from_blog'] == 'yes' ) {
    ...
    function udesign_exclude_portfolio_cats_from_archive_pages( $query ) {
        ...
        // Check if on frontend and main query is being modified
...
function udesign_check_page_breadcrumbs_option( $option_to_check = 'disable_breadcrumbs' ) {
    ...
    $udesign_page_breadcrumbs_options = get_post_meta( $post_id, '_udesign_page_breadcrumbs_options', true );
...
add_action('udesign_single_post_entry_bottom', 'udesign_nextpage_links');
add_action('udesign_single_portfolio_entry_bottom', 'udesign_nextpage_links');
...
function udesign_style_previous_post_link( $link ) {
    ...
    $title = __( $previous_post->post_title );
...
// Add U-Design own elements/shortcodes to VC
require_once( trailingslashit( get_template_directory() ) . 'lib/vc-elements/u-design-vc-elements.php' ); 
 ...
 */
if( is_admin() && ( function_exists( 'vc_license' ) && method_exists( vc_license(), 'isActivated') && !vc_license()->isActivated() ) ) {
...
function udesign_check_page_layout_option( $option_to_check = 'no_header' ) {
    ...
    return ( !is_search() && is_array( $udesign_page_layout_options ) && in_array( $option_to_check, $udesign_page_layout_options ) ) ? true : false;
}
...
/*
 * The Following will output a notice to users to let them know that they need to update/save the theme's Settings after updating the theme post version 2.10.8
 * 
 */
function udesign_check_theme_version_for_admin_notices() {
    global $udesign_options;
    if ( current_user_can( who_can_edit_udesign_theme_options() ) && version_compare(UDESIGN_VERSION, '2.10.8', '>') && (  !empty( $udesign_options ) && version_compare( $udesign_options['udesign_settings_page_last_saved_version'], '2.10.8', '<=' ) ) ) {

        function udesign_update_theme_settings_admin_notice() { ?>
                <div class="notice notice-warning is-dismissible">
                    <p><strong>IMPORTANT MESSAGE (U-DESIGN THEME):</strong> Looks like you just updated the U-Design theme. Some of the changes applied with this update require that you <strong>save</strong> the <a href="<?php echo esc_url(admin_url( 'admin.php' ).'?page=udesign_options_page'); ?>">U-Design Settings page</a>  
                        in order for the new changes to take effect.</p>
                </div>
            <?php 
        }
        add_action( 'admin_notices', 'udesign_update_theme_settings_admin_notice' );

    }
}
add_action( 'init', 'udesign_check_theme_version_for_admin_notices' );
...
function udesign_add_custom_header_image_to_page() {
    if ( !is_search() ) { // do not display on search results page
