...
jQuery(document).ready(function($) {
    ...
    // Slide bar for the "Global Content Width" option
    var globalContentWidthTxtField = $( "#udesign_content_width" );
    var globalContentWidthSlideBar = $( "#udesign_content_width_slide_bar" );
    globalContentWidthSlideBar.slider({
        range: "max",
        min: 600,
        max: 1600,
        step: 10,
        value: [ globalContentWidthTxtField.val() ],
        slide: function( event, ui ) {
            globalContentWidthTxtField.val( ui.value );
        },
        start: function( event, ui ) {
            if ( globalContentWidthTxtField.hasClass('udesign-error') ) {
                globalContentWidthTxtField.removeClass('udesign-error').addClass('udesign-valid').removeAttr('title').tooltip("disable");
            }
        }
    });
    // When 'Global Content Width' text field input has changed
    globalContentWidthTxtField.on('input propertychange', function() {
        // Update the Global Content Width slide-bar
        globalContentWidthSlideBar.slider( 'value', $( this ).val() );
    });
...
jQuery(document).ready(function($){
    ...
    $("#udesign_options_submit_form").validate({
        ...
        rules: {
            ...
            "udesign_options[udesign_content_width]": {
                required: true,
                range: [600, 1600]
            },
