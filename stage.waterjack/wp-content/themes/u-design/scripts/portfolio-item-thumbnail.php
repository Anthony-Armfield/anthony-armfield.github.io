...
function get_portfolio_item_thumbnail( $curr_post_ID, $num_cols='3', $width='260', $height='160', $do_not_link_adjacent_items = 'no', $echo = true ) {
    ...
    $portfolio_default_thumb = get_template_directory_uri() . '/styles/common-images/portfolio-default-thumb.jpg';
    ...
    $portfolio_item_link = apply_filters( 'udesign_get_portfolio_item_link', get_post_meta($curr_post_ID, 'portfolio_item_link', true) );
    ...
    if ( function_exists('udesign_get_the_image') ) { // the case when "Get The Image" plugin is available (installed and activated)
	    $portfolio_thumb_as_array = udesign_get_the_image( array(
				'meta_key' => array('portfolio_item_thumb'),
				'format' => 'array',
				'size' => 'full',
				'default' => $portfolio_default_thumb,
				'link' => false,
				'scan' => true,
			    ) );
		...
		$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col"> </span>';
		$output .= '<a'.$rel_attr.' href="'.$preview_item.'" title="'.$preview_item_title.'"'.$portfolio_item_link_target.'><img class="hover-opacity" src="'.udesign_process_image( $portfolio_thumb_as_array['src'], $width, $height, true, $portfolio_item_thumb_crop_align, $portfolio_item_thumb_retina ).'" width="'.$width.'" height="'.$height.'" alt="'.$portfolio_thumb_as_array['alt'].'" /></a>';
	    } else { // if preview item is NOT available, generate a thumbnail that is NOT a link
		$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col"> </span>';
		$output .= '<img class="hover-opacity" src="'.udesign_process_image( $portfolio_thumb_as_array['src'], $width, $height, true, $portfolio_item_thumb_crop_align, $portfolio_item_thumb_retina ).'" width="'.$width.'" height="'.$height.'" alt="'.esc_attr__('Preview item is not available!', 'udesign').'" />';
	    }
