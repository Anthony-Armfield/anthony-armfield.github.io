<?php
/*
Plugin Name: WordPress Media Library Folders
...
class MaxGalleriaMediaLib {
  ...
  public function __construct() {
	...
	public function set_global_constants() {	
		...
		define('MAXGALLERIA_MEDIA_LIBRARY_VERSION_NUM', '4.0.8');
...
and pm.meta_key = '_wp_attached_file'";
    ...
    if($row) {
      return $row->ID;
			...
			$this->write_log( __('This folder was not found: ','maxgalleria-media-library') . $relative_dir);			
      return false;
    }
      ...
      <div id="wp-media-grid" class="wrap">                
        ...
        <div class="media-plus-toolbar"><div class="media-toolbar-secondary">  
          ...
          <h2 class='mgmlp-title'><?php _e('Media Library Folders', 'maxgalleria-media-library' ); ?> </h2>    
				    ...
				    <p class="center-text-no-ital">Click here to <a href="<?php echo MLF_TS_URL; ?>" target="_blank">Fix Common Problems</a></p>
        ...
        <!--<div class="mlf-clearfix"></div>-->
          ...
          <div id="mgmlp-library-container">
            ...
            $move_or_copy = ($move_or_copy === 'on') ? true : false;
						echo '  <div class="onoffswitch" help="' . __('Move/Copy Toggle. Move or copy selected files to a different folder. When move is selected, images links in posts and pages will be updated. <span class=\'mlp-warning\'>Images IDs used in Jetpack Gallery shortcodes will not be updated.</span>','maxgalleria-media-library') . '">' . PHP_EOL;
            ...
            echo '    <input type="search" placeholder="Search" id="mgmlp-media-search-input" class="search gray-blue-link">' . PHP_EOL;                                    						
            echo '  </span>' . PHP_EOL;           
    ...
    foreach ($folder_contents as $file_path) {
			...
			if($file_path !== '.DS_Store') {
				...
				if(!strpos($new_attachment, '-uai-')) {  // skip thumbnails created by the Uncode theme
					if(!strpos($new_attachment, '-pdf.jpg')) {  // skip pdf previews
    ...
    $browser = $this->get_browser();
		...
		?>
					...
					<div id="wp-media-grid" class="wrap">                
						...
						<div id="mgmlp-header">		
							<div id='mgmlp-title-area'>
							  <h2 class='mgmlp-title'><?php _e('Support - System Information', 'maxgalleria-media-library' ); ?> </h2>    
